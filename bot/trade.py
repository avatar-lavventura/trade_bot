...
class BotHelper:
    ...
    async def set_leverage(self, symbol):
        try:
            leverage = 1
            market = await helper.exchange.future.market(symbol)
    ...
    async def is_usdt_open_open(self, symbol) -> bool:
    ...
    async def both_side_order(self) -> None:
        _symbol = self.strategy.symbol.replace("/USDT", "USDT")
        if await self.is_usdt_open_open(_symbol):
    ...
    def sport_order(self, quantity, symbol=None, side=None):
        ...
        try:
            ...
            if "Precision is over the maximum defined for this asset" in str(e) or "Filter failure: LOT_SIZE" in str(e):
                ...
                return self.sport_order(quantity)
            else:
                _colorize_traceback(e)
                raise e
    ...
    async def sell(self) -> bool:
        default_type = "future"
        output = await self.symbol_price(self.strategy.symbol, default_type)
        current_price = output["last"]
        initial_amount = INITIAL_USDT_QTY / current_price
    ...
    async def _trade(self, strategy):
        if strategy.market_position == "flat":
            ...
            is_open = False
            if strategy.market == "USDTPERP":
                is_open = await self.is_usdt_open_open(strategy.symbol)
            elif strategy.market == "BTC":
                balances = self.client.get_account()
                for balance in balances["balances"]:
...
if __name__ == "__main__":
    ...
    #     asyncio_loop.run_until_complete(bot.trade())
    # except Exception as e:
    #     _colorize_traceback(e)
