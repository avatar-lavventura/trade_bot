            ...
            if future["incomeType"] in ["REALIZED_PNL", "INSURANCE_CLEAR"]:
                ...
                if local_dt.strftime("%d") != _day:
                    ...
                    if daily_progress != 0.0:
                        ...
                        log(f"{arrow_in} {_daily_progress}$ {arrow_out} | ", color=_color, end="")
                        log(f"{format(100.0 * float(_daily_progress) / TOTAL_BALANCE, '.2f')}%", color=_color)
