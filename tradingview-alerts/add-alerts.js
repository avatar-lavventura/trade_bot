...
export const addAlertsMain = async (configFileName) => {
  ...
  if (config.files.exclude) {
    try {
      ...
      if (blackListRows.length > 0) {
        if (!blackListRows[0].symbol) {
          log.error(
            ...
            }), first line must have at the following header: ${kleur.blue(
              "symbol"
            )}`
          );
          process.exit(1);
      ...
      log.fatal(
        `Unable to open file specified in config: ${config.files.exclude}`
      );
      process.exit(1);
    }
  }
  ...
  if (headless) {
    ...
    log.trace(
      `Go to ${config.tradingview.chartUrl} and wait until networkidle2`
    );
  ...
  const parsedIntervals = config.tradingview?.interval
    ?.toString()
    .split(",") || ["none"];
